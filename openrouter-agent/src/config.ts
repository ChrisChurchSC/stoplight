...
export function loadConfig(overrides: Partial<AgentConfig> = {}, opts?: { skipApiKey?: boolean }): AgentConfig {
  ...
  // Model calls now go directly to Anthropic. Prefer ANTHROPIC_API_KEY; the
  // legacy OPENROUTER_API_KEY is read first only so it stays a harmless no-op
  // fallback (ANTHROPIC wins when both are present).
  ...
  if (!config.apiKey && !opts?.skipApiKey) throw new Error('ANTHROPIC_API_KEY is required.');
  return config;
}

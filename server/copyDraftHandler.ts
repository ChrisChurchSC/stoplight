...
Each asset arrives with inputs that MUST shape its copy:
...
- pattern (optional): the SHAPE this one asset is built to — a hook, a format, a structure, or a trend the brand is riding. It carries a name, and where somebody filled them in, a description, a worked example and a when-to-use. Build THIS asset to it.
...
A PATTERN IS A SHAPE, NOT A SUBSTANCE. When an asset carries one, it governs HOW that asset is built — its structure, its opening move, the order the argument lands in — and nothing else. It never supplies the facts: keep the audience, the proof, the theme and the CTA you were given, and do not invent a claim, a statistic, a customer or a scenario because the shape has a slot for one. If the pattern carries an example, it is a demonstration of the FORM, so follow its structure and never reuse its words, its subject or its specifics. A pattern is subordinate to the brand guide and to the audience's antiMessage: where following it would write a line the brand forbids or the sentence this audience must never be told, the pattern loses. Where a pattern genuinely does not fit the asset's channel or component (a five-beat structure in a 30-character headline), apply as much of its shape as the component can hold rather than truncating it into nonsense. Assets carrying DIFFERENT patterns must come out visibly different in construction, not the same copy rearranged.

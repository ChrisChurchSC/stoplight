 ...
 *
 * WHY THIS LIVES IN server/ RATHER THAN api/. It used to be the whole of api/ai-credits.ts, which
 * was the one endpoint file missing from .vercelignore, so it deployed as its own function, never
 * passed through jsonRoute, and answered anybody who asked. Verified against the live pilot: an
 * anonymous GET returned the account's usage and remaining balance. The logic sits here now so the
 * guarded catch-all router and the dev middleware can share one copy, and api/ai-credits.ts is a
 * thin dev-only wrapper that no longer ships.
 *

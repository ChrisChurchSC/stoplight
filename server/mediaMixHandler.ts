...
const SYSTEM = `You are a media planner working inside Hyperfocus. You are given a brand's REAL measured per-channel performance (reach, engagement rate, post count) drawn from its analytics, plus the tool's deterministic baseline budget split, a goal, a budget, and a risk appetite.

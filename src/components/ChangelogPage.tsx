    ...
    groups: [
      {
        ...
        items: [
          ...
          'Green marks Active, and it reads from across the board: a stripe down the edge of a card on the canvas and on the row in the grid, so a campaign part way through shipping says which half is which without opening anything. Planning stays neutral rather than red — red already means something has gone wrong here, and planning is the ordinary state of nearly every card on a board.',
          'And the numbers show their shape, not just their size. Every reading has been kept rather than overwritten since this landed, so under each figure there is now the line it traced to get there and what it moved since the last time you looked. A post that reached forty thousand people in a day and one still climbing three weeks later are different posts to have made, and a single number says the same thing about both. A first reading draws as a dot and says so: a line through one point would invent a direction nothing measured.',
          'Where a post can be read back, it is. A YouTube video and any web page — a landing page, a case study, a blog post — fill themselves in from the link, and neither needs an account connected or a model key. Instagram, LinkedIn, X and TikTok only ever show a post’s words to the account that owns it, so instead of spinning and failing they say which connection would do it. Reading back fills only what is still empty, so a caption you typed is never replaced by one that was found.',
        ],
      },
    ],
  },
